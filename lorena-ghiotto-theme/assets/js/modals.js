...
function renderGallery(images) {
    ...
    const items = images.map((src, idx) => {
        const safe = (typeof getImgUrl === 'function') ? getImgUrl(src) : encodeURI(src);
...
function openSpaziModal() {
    ...
    const galleryHTML = currentGalleryImages.map((img, idx) => {
        const safeImg = (typeof getImgUrl === 'function') ? getImgUrl(img) : encodeURI(img);
        return `
...
function openLightboxGallery(index) {
    lightboxImages = currentGalleryImages.map(img => (typeof getImgUrl === 'function') ? getImgUrl(img) : encodeURI(img));
    lightboxIndex = index;
